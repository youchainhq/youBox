...
const Schema = require("@youbox/contract-schema");
const temp = require("temp").track();
const path = require("path");
const fs = require("fs");
const Config = require("@youbox/config");
const requireNoCache = require("require-nocache")(module);
const Compile = require("@youbox/compile-solidity/legacy");
...
describe("artifactor + require", () => {
  ...
  const web3 = new YOUChain();
  web3.setProvider(provider);

  before(() => web3.you.net.getId().then(id => (networkID = id)));
  ...
  before(() =>
    web3.you.getAccounts().then(_accounts => {
      accounts = _accounts;
      ...
      .then(
        () =>
          new Promise((accept, reject) =>
            web3.you.getBalance(example.address, (err, balance) => {
  ...
  it("should trigger the fallback function when calling send() (shorthand notation)", () => {
    ...
    return Example.new("1", { gas: 3141592 })
      ...
      .then(triggered => {
        ...
        return example.send(web3.utils.toLu("1", "you"));
      })
      .then(
        () =>
          new Promise((accept, reject) =>

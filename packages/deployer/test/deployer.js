...
const Reporter = require("@youbox/reporters").migrationsV5;
...
describe("Deployer (sync)", function() {
  ...
  const web3 = new YOUChain(provider);

  beforeEach(async function() {
    networkId = await web3.you.net.getId();
    const accounts = await web3.you.getAccounts();
  ...
  // There's a chain like this in the @youbox/core solidity-tests
  ...
  it("waits for confirmations", async function() {
    this.timeout(15000);
    const startBlock = await web3.you.getBlockNumber();
    ...
    const libReceipt = await web3.you.getTransactionReceipt(
      IsLibrary.transactionHash
    );
    const exampleReceipt = await web3.you.getTransactionReceipt(

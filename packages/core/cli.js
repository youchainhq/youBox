...
if (!semver.satisfies(process.version, ">=" + minimumNodeVersion)) {
  console.log(
    "Error: Node version not supported. You are currently using version " +
      ...
      " of Node. YOUBox requires Node v" +
      minimumNodeVersion +
      " or higher."
  );
    ...
    if (err instanceof TaskError) {
      ...
      if (err instanceof TruffleError) {
        ...
        if (error === err.stack) {
          ...
          //analytics by cutting off everything before youbox. Will not properly catch the user's info
          //here if the user has youbox in their name.
          let identifyingInfo = String.raw`${directory.split("youbox")[0]}`;

...
const Deployer = require("@youbox/deployer");
const compile = require("@youbox/compile-solidity/new");
const { shimContract } = require("@youbox/compile-solidity/legacy/shims");
const path = require("path");
const semver = require("semver");
const Native = require("@youbox/compile-solidity/compilerSupplier/loadingStrategies/Native");
...
const SolidityTest = {
    ...
    const { contracts } = await compile.with_dependencies(
      runner.config.with({
        paths: [
          ...
          "youbox/DeployedAddresses.sol", // generated by deployed.js
  ...
  async deployTestDependencies(abstraction, dependencyPaths, runner) {
    ...
    const testAbstractions = testLibraries.map(name =>
      runner.config.resolver.require(`youbox/${name}.sol`)
    );

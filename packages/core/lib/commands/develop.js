...
const command = {
  ...
  help: {
    usage: "youbox develop [--log]",
    options: [
      {
        option: `--log`,
        description:
          `Start/Connect to a YOUBox develop session and log all ` +
          `rpc activity. You will\n                    need to open a ` +
          `different YOUBox develop or console session to interact via the repl.`
      }
    ]
  },
  runConsole: (config, ganacheOptions, done) => {
    const Console = require("../console");
    const { Environment } = require("@youbox/environment");
  ...
  run: (options, done) => {
    const { Develop } = require("@youbox/environment");
    ...
    const warning =
      ...
      "This mnemonic was created for you by YOUBox. It is not secure.\n" +
    ...
    function sanitizeNetworkID(network_id) {
      if (network_id !== "*") {
        if (!parseInt(network_id, 10)) {
          const error =
            `The network id specified in the youbox config ` +
    ...
    Develop.connectOrStart(ipcOptions, ganacheOptions, started => {
      ...
      if (started) {
        config.logger.log(`YOUBox Develop started at ${url}`);
        config.logger.log();
        ...
        config.logger.log(
          `Connected to existing YOUBox Develop session at ${url}`
        );
        config.logger.log();
      }

...
const provision = require("@youbox/provisioner");
const {
  Web3Shim,
  createInterfaceAdapter
} = require("@youbox/interface-adapter");
  ...
  start(callback) {
    ...
    try {
      this.interfaceAdapter.getAccounts().then(fetchedAccounts => {
        ...
        this.repl.start({
          prompt: "youbox(" + this.options.network + ")> ",
          context: {
            youchain: this.youchain,

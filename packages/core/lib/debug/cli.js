...
const Debugger = require("@youbox/debugger");
const DebugUtils = require("@youbox/debug-utils");

const { DebugInterpreter } = require("./interpreter");
const { DebugCompiler } = require("./compiler");
const { shimBytecode } = require("@youbox/compile-solidity/legacy/shims");
...
class CLIDebugger {
  ...
  async run(txHash) {
    this.config.logger.log("Starting YOUBox Debugger...");

    // compile contracts
    const compilation = this.compilation || (await this.compileSources());

    // invoke @youbox/debugger

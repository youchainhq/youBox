...
const Networks = {
  ...
  display: async function(config) {
    ...
    if (
      ...
      config.logger.log(
        OS.EOL +
          "Closely inspect the deployed networks below, and use `youbox networks --clean` to remove any networks that don't match your configuration. You should not use the wildcard configuration ('*') for staging and production networks for which you intend to deploy your application."
      );
    }

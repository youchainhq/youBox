 ...
 * primarily for internal use by YOUBox and so remain largely undocumented,
 * but some of its types are also output by @youbox/decoder, which provides
 * a higher-level interface to much of this module's functionality.
 *
 * ## If you're here from YOUBox Decoder
 *
 * If you're coming here from [[@youbox/decoder]], you probably just want to
 ...
 *
 * If you're not just here from YOUBox Decoder, but are actually
 * interested in the lower-level workings, read on.
 *
 * ## How this module differs from YOUBox Decoder
 *
 * Unlike YOUBox Decoder, this library makes no network connections
 ...
 * connection of its own.  This is how @youbox/decoder works; @youbox/codec
 * makes requests, and @youbox/decoder fulfills them by
 ...
 * by YOUBox Decoder.  In particular, this library also exists to
 * support YOUBox Debugger, and so it provides encoding functionality not
 ...
 * even though YOUBox Decoder does not.
 *
 ...
 * ## How to use
 *
 * You should probably use [[@youbox/decoder]] instead, if your use case doesn't
 ...
 * YOUBox Decoder can perhaps serve as something of a reference implementation
 * (and perhaps YOUBox Debugger as well, though that code is much harder to
 * read or copy).
 *
 * @module @youbox/codec
 */ /** */
...
`@youbox/decoder@^4.0.0` and `@youbox/codec@^0.1.0`.
...
import * as Codec from "@youbox/codec";

// or import Format directly:
import { Format } from "@youbox/codec";
```
...
within the context of the [YOUChain Virtual Machine](https://ethereum.github.io/yellowpaper/paper.pdf)
(EVM) and in raw data for transactions and logs according to the
[YOUChain JSON RPC](https://github.com/YOUChain/wiki/wiki/JSON-RPC).
...
[[Format.Values.Result]], which contain the following fields:
  ...
  5. `reference`: This field is a debugger-only feature and does not
     apply to results returned by  @youbox/decoder, so it won't be documented here.
...
decoded via @youbox/decoder.  However, they decode to a bare-bones value,
...
will not include the function name or defining class, as @youbox/decoder
...
error.  However, when using @youbox/decoder, we have no way of discerning whether

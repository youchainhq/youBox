...
import {
  ...
  LogDecoding,
  DecoderOptions
} from "@youbox/codec/types";
import * as Evm from "@youbox/codec/evm";
import * as Contexts from "@youbox/codec/contexts";
import { abifyType, abifyResult } from "@youbox/codec/abify";
import * as Conversion from "@youbox/codec/conversion";
import * as Format from "@youbox/codec/format";
import { StopDecodingError } from "@youbox/codec/errors";
import read from "@youbox/codec/read";
import decode from "@youbox/codec/decode";
    ...
    if (allocation.kind === "bytecode") {
      ...
      if (contractType.contractKind === "library") {
        ...
        if (context.binary.startsWith(delegateCallGuardString)) {
          decoding.address = YOUChainUtils.toChecksumAddress(

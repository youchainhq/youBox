...
describe("Methods", function() {
  ...
  describe(".method(): success [ @geth ]", function() {
    ...
    it("should honor the defaultBlock parameter when called", async function() {
      ...
      const initialBlock = await web3.you.getBlockNumber();
  ...
  describe(".method(): errors [ @geth ]", function() {
    ...
    it("errors with a revert reason", async function() {
      const example = await Example.new(1);
      try {
        // At the moment, this test can't rely on @youbox/contract's
  ...
  describe.skip("web3 wallet", function() {
    it("should work with a web3.accounts.wallet account", async function() {
      ...
      const wallet = web3.you.accounts.wallet.create(1);
      const providerAccounts = await web3.you.getAccounts();
      await web3.you.sendTransaction({
        from: providerAccounts[0],
        to: wallet["0"].address,
        value: web3.utils.toLu("1", "you")
      });

      const balance = await web3.you.getBalance(wallet["0"].address);
      assert.equal(balance, web3.utils.toLu("1", "you"));
  ...
  describe("sendTransaction() / send() [ @geth ]", function() {
    it("should trigger the fallback function when calling sendTransaction()", async function() {
      ...
      const balance = await web3.you.getBalance(example.address);
      assert(
        balance === web3.utils.toLu("1", "you"),
        "Balance should be 1 you"
      );
    });
    ...
    it("should trigger the fallback function when calling send() (shorthand notation)", async function() {
      ...
      await example.send(web3.utils.toLu("1", "you"));

      const balance = await web3.you.getBalance(example.address);
      assert(balance === web3.utils.toLu("1", "you"));
    });

    it("should accept tx params (send)", async function() {
      const example = await Example.new(1);
      const eth = web3.utils.toLu("1", "you");
      const sender = accounts[1];
      const initialSenderBalance = await web3.you.getBalance(sender);

      await example.send(eth, { from: sender });

      const finalSenderBalance = await web3.you.getBalance(sender);
      const contractBalance = await web3.you.getBalance(example.address);

...
const abi = require("youchain-you-abi");
...
const Utils = {
  ...
  is_big_number(val) {
    ...
    //YOUChainUtils.isBigNumber returns true not only for
    //bignumber.js BigNumbers, but also for ethers BigNumbers,
    //even though these are totally different things.
    return YOUChainUtils.isBN(val) || YOUChainUtils.isBigNumber(val);
  },
  ...
  convertToEthersBN(original) {
    ...
    original.forEach(item => {
      ...
      if (Array.isArray(item)) {
        ...
        //HACK: Since we can't rely on YOUChainUtils.isBigNumber to tell

...
const handlers = {
  ...
  maxConfirmations: 24, // Maximum number of confirmation youchainjs emits
  defaultTimeoutBlocks: 50, // Maximum number of blocks youchainjs will wait before abandoning tx
  timeoutMessage: "50 blocks", // Substring of youchainjs timeout error.
  ...
  /**
   * Attaches YOUBox specific handlers to all of the events emitted by a youchainjs method.
   * @param {Object}       context  execution state
   * @param {PromiEvent}   emitter  promiEvent returned by a youchainjs method call
   */
  ...
  error: function(context, error) {
    if (!handlers.ignoreTimeoutError(context, error)) {
      ...
      this.removeListener && this.removeListener("error", handlers.error);
    }
  },
  ...
  confirmation: function(context, number, receipt) {
    ...
    // Per youchainjs: initial confirmation index is 0
  ...
  receipt: async function(context, receipt) {
    ...
    this.removeListener && this.removeListener("receipt", handlers.receipt);
  }
};

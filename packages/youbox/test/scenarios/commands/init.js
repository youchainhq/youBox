...
describe("youbox init [ @standalone ]", () => {
  let tempDir, config;
  ...
  it("unboxes a project with a youbox config", done => {
  ...
  describe("when a path is provided", () => {
    it("initializes with a relative path", done => {
      ...
      CommandRunner.run(`init ${myPath}`, config, error => {
        if (error) done(error);
        assert(
          fse.existsSync(path.join(tempDir.name, myPath, "youbox-config.js"))
        );
        done();
      });
    }).timeout(20000);
    it("initializes with an absolute path", done => {
      ...
      CommandRunner.run(`init ${myPath}`, config, error => {
        if (error) done(error);
        assert(fse.existsSync(path.join(myPath, "youbox-config.js")));
        done();
      });
    }).timeout(20000);
  });

...
import { ContractObject as Artifact } from "@youbox/contract-schema/spec";
...
export class WireDecoder {
   ...
   * @protected
   */
  constructor(contracts: Artifact[], provider: any) {
  ...
  public async getCode(
    ...
    if (block === "pending") {
      return Conversion.toBytes(await this.web3.you.getCode(address, block));
    }
    ...
    let code = Conversion.toBytes(await this.web3.you.getCode(address, block));
  ...
  public async regularizeBlock(
    ...
    return (await this.web3.you.getBlock(block)).number;
  }
  ...
  public async decodeLogWithAdditionalOptions(
    ...
    const topics = log.topics.map(item => {
      if (typeof item === "string") {
        return Conversion.toBytes(item);
      } else if (item) {
        //TODO  topics: string[];   => topics: Array<string | string[]>;
        //return item.map(Conversion.toBytes);
      }
    });
    const info: Evm.EvmInfo = {
      state: {
  ...
  public async eventsWithAdditionalContexts(
    ...
    const logs = await this.web3.you.getPastLogs({
      address,
...
export class ContractDecoder {
  ...
  public async init(): Promise<void> {
    this.contractNetwork = (await this.web3.you.net.getId()).toString();
  }
...
export class ContractInstanceDecoder {
  ...
  public async init(): Promise<void> {
    this.contractCode = Conversion.toHexString(
      await this.getCode(
        ...
        await this.web3.you.getBlockNumber() //not "latest" because regularized
      )
    );
  ...
  public async state(
    ...
    return {
      ...
      balanceAsBN: new BN(
        await this.web3.you.getBalance(this.contractAddress, blockNumber)
      ),
      nonceAsBN: new BN(
        await this.web3.you.getTransactionCount(
          this.contractAddress,
          blockNumber
        )
      )
  ...
  private async getStorage(
    ...
    if (block === "pending") {
      return Conversion.toBytes(
        await this.web3.you.getStorageAt(address, slot.toNumber(), block),

...
async function verifyURL(url: string) {
  ...
  const configURL = parseURL(
    `${vcsurl.vcsurl(url).replace(/#.*/, "")}/blob/master/youbox-box.json`
  );
  ...
  try {
    ...
    if (statusCode === 404) {
      throw new Error(
        `YOUBox Box at URL ${url} doesn't exist. If you believe this is an error, please contact YOUBox support.`
      );
    ...
    console.log("\nyoubox-box.json", e.message, "options", options);
  }
}
...
function prepareToCopyFiles(tempDir: string, { ignore }: boxConfig) {
  ...
  needingRemoval.push("youbox-box.json");
  needingRemoval.push("youbox-init.json");

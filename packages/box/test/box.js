...
describe("@youbox/box Box", () => {
  ...
  describe(".unbox()", () => {
    ...
    it("unboxes youbox box from github", done => {
      Box.unbox(TRUFFLE_BOX_DEFAULT, destination, {}, config).then(
        truffleConfig => {
          assert.ok(truffleConfig);

          assert(
            fse.existsSync(path.join(destination, "youbox-config.js")),
            "Unboxed project should have youbox config."
          );
          done();
        }
      );
  ...
  describe("--force", () => {
    ...
    it("unboxes youbox box when used", done => {
    ...
    it("overwrites redundant files if init/unbox force flag used", done => {
      const truffleConfigPath = path.join(destination, "youbox-config.js");

      // preconditions
      fse.writeFileSync(
        truffleConfigPath,
        "this youbox-config.js file is different than the default box file",
        "utf8"
      );
      assert(
        fse.existsSync(truffleConfigPath),
        "mock youbox-config.js wasn't created!"
      );

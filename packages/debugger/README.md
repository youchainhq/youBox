...
Portable Solidity debugger library, for use with or without YOUBox.
...
Please see the [reference integration](https://github.com/trufflesuite/truffle/tree/master/packages/core/lib/commands/debug.js) provided by the `youbox debug` command._

### Required Parameters

To start a @youbox/debugger session, you'll need the following:
...
- `files` - An array of sourcePaths representing file indexes (from `solc` or `@youbox/compile-solidity`)
...
```javascript
import Debugger from "@youbox/debugger";

...
if [ "$INTEGRATION" = true ]; then
  ...
  lerna run --scope youbox test --stream
  ...
  run_geth
  sleep 30
  lerna run --scope youbox test --stream -- --exit
  lerna run --scope @youbox/contract test --stream -- --exit
  ...
  youbox version
  youbox compile --compilers.solc.parser=solcjs && youbox compile --compilers.solc.parser=solcjs --contracts_directory 'lib/dappsys/[!note][!stop][!proxy][!thing][!token]*.sol' && bash ./scripts/provision-token-contracts.sh
  npm run start:blockchain:client & youbox migrate --compilers.solc.parser=solcjs --reset --compile-all && youbox test --compilers.solc.parser=solcjs ./test/contracts-network/* ./test/extensions/* --network development
  ...
  lerna run --ignore youbox test --stream --concurrency=1
